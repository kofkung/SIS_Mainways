...
export function MagicCard(props: MagicCardProps) {
  ...
  return (
    ...
    >
      <div className="magic-card-backdrop bg-background absolute inset-px z-20 rounded-[inherit]" />

      {mode === "gradient" && (
        <motion.div
          suppressHydrationWarning
          className="magic-card-gradient pointer-events-none absolute inset-px z-30 rounded-[inherit] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          style={{
            background: useMotionTemplate`
              radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
                ${gradientColor},
      ...
      <div className="magic-card-content relative z-40">{children}</div>
    </motion.div>
  )
}
